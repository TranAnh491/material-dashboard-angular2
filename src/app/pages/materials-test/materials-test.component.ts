...
@Component({
  selector: 'app-materials-test',
  templateUrl: './materials-test.component.html',
  styleUrls: ['./materials-test.component.scss']
})
export class MaterialsTestComponent implements OnInit, AfterViewInit {
  isLoading = false;
  ...
  testData: TestData = {
    ...
    sections: [
      {
        title: "PHẦN I: QUY TRÌNH NHẬP KHO NGUYÊN VẬT LIỆU",
        questions: [
          {
            question: "Khi nhận hàng nguyên vật liệu, bước đầu tiên cần làm là:",
            options: [
          ...
          {
            question: "Khi phát hiện hàng hóa không đạt chất lượng, cần làm gì?",
            options: [
          ...
          {
            question: "Khi nhập kho nguyên vật liệu, cần kiểm tra chứng từ gì?",
            options: [
              "Chỉ phiếu nhập kho",
              "Chỉ hóa đơn",
          ...
          {
            question: "Sau khi nhập kho, nguyên vật liệu cần được:",
            options: [
      ...
      {
        title: "PHẦN II: QUY TRÌNH XUẤT KHO NGUYÊN VẬT LIỆU",
        questions: [
          {
            question: "Khi xuất kho nguyên vật liệu, cần kiểm tra gì trước?",
            options: [
              "Chỉ kiểm tra số lượng",
          ...
          {
            question: "Nguyên tắc xuất kho là:",
            options: [
          ...
          {
            question: "Khi xuất kho, cần ghi chép gì?",
            options: [
              "Chỉ ghi số lượng",
              "Chỉ ghi ngày xuất",
          ...
          {
            question: "Nếu phát hiện hàng hóa hết hạn khi xuất kho, cần làm gì?",
            options: [
              "Xuất bình thường",
  ...
  private async saveTestResult(result: TestResult): Promise<void> {
    try {
      ...
      // Save to materials test results
      ...
      const docRef = await addDoc(collection(this.db, 'materials-test-results'), testResultData);
  ...
  private createPDFContent(): string {
    ...
    return `
      <div style="text-align: center; margin-bottom: 30px;">
        ...
        <h3 style="margin: 0 0 20px 0; font-size: 14px; font-weight: bold; color: #34495e;">HƯỚNG DẪN XUẤT NHẬP KHO NGUYÊN VẬT LIỆU</h3>
      </div>
  ...
  private splitContentIntoPages(content: string): string[] {
    // For Materials Test with 12 questions, split into 2 pages: 6 questions each
