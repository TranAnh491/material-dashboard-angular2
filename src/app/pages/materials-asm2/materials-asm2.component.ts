...
export class MaterialsASM2Component implements OnInit, OnDestroy, AfterViewInit {
  ...
  kkCheckSelectedFactory: 'ASM1' | 'ASM2' | 'ASM3' = 'ASM2';
  kkCheckSearchCode = '';
  ...
  openTemThungPopup(): void {
    ...
    setTimeout(() => {
      const el = document.getElementById('tem-thung-code-input-asm2') as HTMLInputElement | null;
      el?.focus();
      el?.select();
    }, 50);
  }
  ...
  private static readonly CATALOG_LOCALSTORAGE_KEY = 'materials-asm2-catalog-cache-v1';
  ...
  private tryLoadCatalogFromLocalStorage(): boolean {
    try {
      const raw = localStorage.getItem(MaterialsASM2Component.CATALOG_LOCALSTORAGE_KEY);
      if (!raw) return false;
      const parsed = JSON.parse(raw) as { items?: any[]; timestamp?: number };
      if (!parsed?.items?.length || !parsed.timestamp) return false;
      if (Date.now() - parsed.timestamp >= MaterialsASM2Component.CATALOG_CACHE_TTL_MS) return false;
  ...
  private saveCatalogToLocalStorage(): void {
    try {
      ...
      localStorage.setItem(
        MaterialsASM2Component.CATALOG_LOCALSTORAGE_KEY,
  ...
  private async loadCatalogFromFirebase(): Promise<void> {
    ...
    // (dữ liệu catalog ít thay đổi, cache 12 tiếng là an toàn).
    ...
    try {
      ...
      if (snapshot && !snapshot.empty) {
        ...
        this.catalogLoaded = true;
        this.readTracker.track('materials-asm2', collectionName, snapshot.size);
  ...
  async resetZeroStock(): Promise<void> {
    ...
    try {
      ...
      alert(`❌ Lỗi khi reset ASM2: ${error?.message || error}`);
    } finally {
  ...
  private async performResetInventory(): Promise<void> {
    console.log('📡 Querying all ASM2 materials from Firebase...');
    const snapshot = await this.firestore
    ...
    if (!snapshot || snapshot.empty) {
      alert('✅ Không có mã hàng nào trong ASM2');
      return;
    }
    ...
    if (zeroStockIds.length === 0 && lowStockRows.length === 0) {
      alert('✅ Không có mã tồn = 0 hoặc tồn < 1 trong ASM2');
      return;
    }
    ...
    if (zeroStockIds.length > 0) {
      ...
      console.log(`✅ ASM2: deleted ${deletedZero} items with zero stock`);
    }
  ...
  async printCustomLabels(): Promise<void> {
    ...
    try {
      ...
      this.openCustomLabelPrintWindow(printWindow, qrImages, code, imd);
    } catch (e: any) {
      console.error('[Custom Print ASM2] error', e);
  ...
  private openCustomLabelPrintWindow(
    printWindow: Window,
    qrImages: Array<{ image: string; qrData: string; index: number }>,
    code: string,
    imdInput: string
  ): void {
        ...
        <div class="qr-container">
          ...
          <div class="info-section">
            <div>
              ...
              <div class="info-row material-code">${this.formatInboundLabelQuantity(p.quantity)}</div>
              ${imdInput ? `<div class="info-row">IMD: ${esc(p.imd)}</div>` : ''}
...
<html><head><title>In Tùy Chỉnh – ASM2 – ${esc(code)}</title>
<style>
