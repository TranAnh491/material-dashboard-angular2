...
import { sendZaloPhotoByUrl, sendZaloToChat } from './zalo-notify.util';

export const ZALO_KHO_GROUP_DOC = 'zalo_group_config/quanly_kho';

const GUIDE_IMAGES: Array<{ file: string; caption: string }> = [
...
export const KK_SCAN_GUIDE_TEXT = `HƯỚNG DẪN SCAN — ĐỌC CHẬM, LÀM TỪNG BƯỚC
...
async function resolveKhoGroupChatId(db: admin.firestore.Firestore): Promise<string> {
  const doc = await db.doc(ZALO_KHO_GROUP_DOC).get();
  const chatId = String(doc.data()?.chatId || '').trim();
  if (chatId) return chatId;
  throw new Error('Chưa gắn nhóm Quản lý kho trong zalo_group_config/quanly_kho.');
}

function loadGuideImageBuffer(file: string): Buffer | null {
...
async function guideImagePublicUrl(file: string, buf: Buffer): Promise<string | null> {
  const hosting = `${HOSTING_IMG}/${file}`;
  try {
    const res = await fetch(hosting, { method: 'GET' });
    const ctype = String(res.headers.get('content-type') || '');
    if (res.ok && ctype.startsWith('image/')) return hosting;
  } catch {
    /* fall through */
  }
  try {
...
function splitGuideText(text: string, max = 1900): string[] {
  const parts: string[] = [];
  let rest = text.trim();
...
export async function sendKkScanGuideToKhoGroup(
  db: admin.firestore.Firestore,
  sentBy?: string
): Promise<{ ok: true; images: number }> {
  ...
  for (const img of GUIDE_IMAGES) {
    ...
    const ok = await sendZaloPhotoByUrl(chatId, url, img.caption);
    if (ok) images += 1;
    else console.warn('kk-scan-guide: sendPhoto failed', img.file);
  }
  ...
  const intro =
    '📘 Hướng dẫn Scan kiểm kê (đọc chậm, làm từng bước).' +
    (who ? `\nNgười gửi: ${who}` : '');
  await sendZaloToChat(chatId, intro);
  for (const part of splitGuideText(KK_SCAN_GUIDE_TEXT)) {
    await sendZaloToChat(chatId, part);
  }
