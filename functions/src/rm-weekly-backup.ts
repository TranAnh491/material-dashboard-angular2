...
async function backupCollectionToStorage(
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    const stream = db.collection(name).stream() as NodeJS.ReadableStream;
    stream.on('error', reject);
    stream.on('data', (doc: admin.firestore.QueryDocumentSnapshot) => {
      let line: string;
      try {
...
async function listExistingBackupDates(
  bucket: ReturnType<admin.storage.Storage['bucket']>
): Promise<string[]> {
  const [files] = await bucket.getFiles({ prefix: `${BACKUP_ROOT}/` });
  ...
  for (const f of files) {
    ...
    if (/^\d{4}-\d{2}-\d{2}$/.test(seg)) dates.add(seg);
  }
  return [...dates].sort();
}
...
async function sendBackupSummaryEmail(
  date: string,
  ...
  const rows = results
    .map(
      (r) =>
        `<tr><td>${r.name}</td><td style="text-align:right">${r.count.toLocaleString('vi-VN')}</td>` +
  ...
  const text =
    `Backup hàng tuần dữ liệu kho NVL (Inbound → Tồn → Outbound).\n\n` +
    `Ngày: ${date}\n` +
...
export interface RmWeeklyBackupResult {
  date: string;
  totalDocs: number;
...
export async function runRmWeeklyBackupJob(): Promise<RmWeeklyBackupResult> {
