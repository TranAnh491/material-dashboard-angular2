...
exports.requestMaterialsInventoryOtp = requestMaterialsInventoryOtp;
exports.verifyMaterialsInventoryOtp = verifyMaterialsInventoryOtp;
/**
...
async function sendOtpToZalo(db, memberId, code, token, requestedBy, actionLabel, factory) {
    var _a;
    ...
    const msg = `🔐 Yêu cầu thao tác tồn kho (RM Inventory)\n` +
        `Thời điểm: ${atStr}\n` +
        requesterLine +
        factoryLine +
        actionLine +
...
async function requestMaterialsInventoryOtp(db, opts) {
    ...
    const actionLabel = String((opts === null || opts === void 0 ? void 0 : opts.actionLabel) || '').trim().slice(0, 120);
...
async function verifyMaterialsInventoryOtp(db, codeRaw) {
    var _a, _b, _c;
...
//# sourceMappingURL=materials-inventory-otp.js.map