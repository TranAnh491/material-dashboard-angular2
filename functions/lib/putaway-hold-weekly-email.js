...
exports.HOLD_NOTIFICATION_EMAILS_DOC = void 0;
exports.loadHoldNotificationEmails = loadHoldNotificationEmails;
exports.runPutawayHoldWeeklyEmail = runPutawayHoldWeeklyEmail;
const admin = __importStar(require("firebase-admin"));
const nodemailer = __importStar(require("nodemailer"));
const params_config_1 = require("./params-config");
exports.HOLD_NOTIFICATION_EMAILS_DOC = 'qc-settings/hold-notification-emails';
function esc(s) {
...
function isIqcStagingLocation(locationRaw) {
    return (locationRaw || '').trim().toUpperCase().startsWith('IQC');
}
function normalizePutawayIqcStatus(raw) {
    const s = (raw || '').trim();
    if (!s)
        return null;
    ...
    if (u === 'NG')
        return 'ng';
    if (u.includes('LOCK') || u.includes('KHÓA') || u.includes('KHOA'))
        return 'lock';
    if (u === 'HOLD' || u.includes('ĐẶC CÁCH') || u.includes('DAC CACH'))
        return 'confirm';
    if (u.includes('CHỜ XÁC NHẬN') || u.includes('CHO XAC NHAN'))
        return 'confirm';
    if (u.includes('CHỜ KIỂM') || u.includes('CHỜ KIỂM TRA') || u.includes('CHO KIEM'))
        return 'pending';
    const compact = u.replace(/\s+/g, '');
    if (compact.includes('CHỜXÁCNHẬN') || compact.includes('CHOXACNHAN'))
        return 'confirm';
    if (compact.includes('CHỜKIỂM') || compact.includes('CHỜKIỂMTRA') || compact.includes('CHOKIEM'))
        return 'pending';
    return null;
}
function mergePutawayStatusKind(a, b) {
    const rank = { lock: 5, ng: 4, confirm: 3, pending: 2, pass: 1 };
    return rank[a] >= rank[b] ? a : b;
}
function parseInventoryDate(data) {
    const fields = ['importDate', 'lastUpdated', 'createdAt'];
    for (const key of fields) {
        const raw = data === null || data === void 0 ? void 0 : data[key];
        if (!raw)
            continue;
        ...
        if (raw instanceof admin.firestore.Timestamp)
            return raw.toDate();
        if (raw === null || raw === void 0 ? void 0 : raw.toDate)
            return raw.toDate();
        if (raw === null || raw === void 0 ? void 0 : raw.seconds)
...
function getIMDFromDate(date) {
    const day = String(date.getDate()).padStart(2, '0');
    const month = String(date.getMonth() + 1).padStart(2, '0');
    const year = String(date.getFullYear());
    return `${day}${month}${year}`;
}
function countDaysNoSunday(from, to) {
    if (to <= from)
        return 0;
    let count = 0;
...
function computeDayInIqc(materialDate) {
...
async function fetchPutawayStagingInventoryDocs(db, factory) {
    const mergeSnapshotsByDocId = (snaps) => {
        const byId = new Map();
    ...
    return fb.docs.filter((d) => { var _a; return isIqcStagingLocation(String(((_a = d.data()) === null || _a === void 0 ? void 0 : _a.location) || '')); });
}
function collectHoldSkuLines(factory, docs) {
    const skuLines = new Map();
    for (const doc of docs) {
        const data = doc.data();
        const locRaw = String(data.location || '').trim();
        if (!isIqcStagingLocation(locRaw))
            continue;
    ...
    skuLines.forEach((line) => {
        if (!line.materialDate)
            return;
        const prev = earliestByCode.get(line.materialCode);
        if (!prev || line.materialDate < prev)
            earliestByCode.set(line.materialCode, line.materialDate);
    });
    return Array.from(skuLines.values())
        ...
        .sort((a, b) => {
        if (b.dayInIqc !== a.dayInIqc)
            return b.dayInIqc - a.dayInIqc;
        if (a.materialCode !== b.materialCode)
            return a.materialCode.localeCompare(b.materialCode);
        return a.poNumber.localeCompare(b.poNumber);
    });
}
function buildHoldMaterialRows(skuLines) {
    ...
    for (const line of skuLines) {
        ...
        if (line.dayInIqc > bucket.dayInIqc)
            bucket.dayInIqc = line.dayInIqc;
...
async function loadHoldNotificationEmails(db) {
    const snap = await db.doc(exports.HOLD_NOTIFICATION_EMAILS_DOC).get();
...
async function loadHoldDataForFactories(db, factories) {
    const allSku = [];
...
function buildHoldEmailHtml(materialRows, skuLines, atStr) {
...
function buildHoldEmailText(materialRows, skuLines, atStr) {
    const summary = materialRows
        .map((r) => `  • [${r.factory}] ${r.materialCode} — Day ${r.dayInIqc}, Hold SKU: ${r.holdCount}, Tồn: ${r.totalStock.toFixed(2)}`)
        .join('\n');
    const detail = skuLines
        .map((r) => `  • [${r.factory}] ${r.materialCode} | PO ${r.poNumber || '—'} | IMD ${r.imd} | ${r.location} | ${r.iqcStatus} | Tồn ${r.stock.toFixed(2)} | Day ${r.dayInIqc}`)
        .join('\n');
    return (`Putaway — Báo cáo mã hàng đang Hold\n\n` +
        `Thời điểm: ${atStr}\n` +
        `Tổng: ${materialRows.length} mã hàng, ${skuLines.length} SKU Hold.\n\n` +
        `--- Tóm tắt ---\n${summary || '  (không có)'}\n\n` +
        `--- Chi tiết SKU ---\n${detail || '  (không có)'}\n`);
}
async function runPutawayHoldWeeklyEmail(db) {
    ...
    console.log(`[putaway-hold] Đã gửi mail tới ${recipients.length} người nhận. Hold: ${materialRows.length} mã, ${skuLines.length} SKU.`);
    return {
        sent: true,
...
//# sourceMappingURL=putaway-hold-weekly-email.js.map