...
function compositeKey(factory, materialCode, poNumber, imd, bichBatch, bagDisplay) {
...
async function scanOutboundDuplicates(db, cached) {
    ...
    for (const { count, sample, lsx, latestMs } of counts.values()) {
        if (count > 1) {
            ...
            const dupKey = compositeKey(sample.factory, sample.materialCode, sample.poNumber, sample.imd, sample.bagBatch, String((_f = sample.bagNumberDisplay) !== null && _f !== void 0 ? _f : ''));
            const ignoredBaseline = s.ignoredGroups.get(dupKey);
            if (ignoredBaseline != null && ignoredBaseline >= count) {
                continue;
            }
            dupes.push(Object.assign(Object.assign({}, sample), { latestExportAtMs: latestMs || 0, latestExportAtLabel: fmtVn(latestMs) || undefined, productionOrderFirst: (lsxList === null || lsxList === void 0 ? void 0 : lsxList[0]) || '', count,
...
async function resolveZaloChatId(db, memberId) {
    ...
    if (snap.empty)
        return null;
    const chatId = String(((_a = snap.docs[0].data()) === null || _a === void 0 ? void 0 : _a.chatId) || '').trim();
    return chatId || null;
}
async function sendZaloText(token, chatId, text) {
...
function buildZaloDupMessage(dupes, dupSinceLabel, contextLabel) {
    const MAX_LINES = 15;
    const lines = dupes
        .slice(0, MAX_LINES)
        .map(r => `• ${r.factory} ${r.materialCode} PO ${r.poNumber} IMD ${r.imd || '—'} Bịch ${r.bagBatch || '—'} Bag ${r.bagNumberDisplay || '—'} — ${r.count} lần`);
    const more = dupes.length > MAX_LINES ? `\n… và ${dupes.length - MAX_LINES} nhóm khác` : '';
    return (`🚨 [Control Batch] ${dupes.length} nhóm trùng xuất kho (${contextLabel})\n` +
 ...
 */
async function sendDupZaloAlert(db, dupes, dupSinceLabel, contextLabel) {
    if (dupes.length === 0)
        return;
    try {
