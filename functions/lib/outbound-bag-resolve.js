...
function collectOutboundBagParseCandidates(d) {
    var _a;
    const out = [];
    const seen = new Set();
    const push = (x) => {
...
function resolveControlBatchOutboundRowBagFields(d) {
    let bagBatchNorm = d['bagBatch'] != null && String(d['bagBatch']).trim() !== '' ? String(d['bagBatch']).trim() : '';
    ...
    const parsedNumOnlySameAsNumerator = den != null &&
        ...
        !hasSplitTag &&
        bagUi.length > 0 &&
        !!(parsedUsedForBagNum === null || parsedUsedForBagNum === void 0 ? void 0 : parsedUsedForBagNum.bagNumberDisplay) &&
