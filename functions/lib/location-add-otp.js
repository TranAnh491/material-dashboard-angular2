...
exports.requestLocationAddOtp = requestLocationAddOtp;
exports.verifyLocationAddOtp = verifyLocationAddOtp;
...
async function sendOtpToZalo(db, memberId, code, token, requestedBy, locationName) {
    var _a;
    ...
    const msg = `🔐 Yêu cầu thêm vị trí mới (Location)\n` +
        `Thời điểm: ${atStr}\n` +
        requesterLine +
        locationLine +
        `Mã xác nhận: ${code}\n` +
...
async function requestLocationAddOtp(db, requestedByRaw, locationNameRaw) {
...
async function verifyLocationAddOtp(db, codeRaw) {
    var _a, _b, _c;
...
//# sourceMappingURL=location-add-otp.js.map